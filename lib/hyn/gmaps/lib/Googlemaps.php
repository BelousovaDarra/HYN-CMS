...
class Googlemaps {
	...
	var $https						= HYN_URI_SSL;					// If set to TRUE will load the Google Maps JavaScript API over HTTPS, allowing you to utilize the API within your HTTPS secure application 
	...
	function create_map()
	{
		...
		$apiLocation .= '.google.com/maps/api/js?sensor='.($this->sensor ? "true" : "false" );
		if ($this->apikey) { $apiLocation .= "&key=".$this -> apikey; }
		...
		DOM::set_js( $apiLocation );
        		
		if ($this->center=="auto" || $this->directionsStart=="auto") {
			DOM::set_js( "http". ( $this -> https ? "s" : false ) ."://code.google.com/apis/gears/gears_init.js" );
		}
		if ($this->cluster) {
			DOM::set_js( "http". ( $this -> https ? "s" : false ) ."://google-maps-utility-library-v3.googlecode.com/svn/trunk/markerclusterer/src/markerclusterer_compiled.js" );
		}
		...
		DOM::add_js( $this -> output_js );
		return array('html'=>$this->output_html, 'markers'=>$this->markersInfo);
